...
type ItemPropsType = {
	...
	device: string;
	choices: any;
	remove: ( row: number, area: string, item: any ) => void;
	customizer: any;
	[key: string]: any;
}
...
const Item: React.FC<ItemPropsType> = ( {
	...
	return (
		...
		<div
			className={ `vite-builder-component${ isActive ? ' is-active' : '' }` }
			// @ts-ignore
...
const Builder: React.FC<any> = ( props ) => {
	const {
		control: {
			id,
			params: {
	...
	const Portal: React.FC<any> = usePortal( document.querySelector( '.wp-full-overlay' ) as HTMLElement );
	...
	useEffect( () => {
		...
		for ( const pos of [ 'top', 'main', 'bottom' ] ) {
			...
			rowSetting?.bind( ( v: any ) => {
				setCol( ( prev: any ) => ( { ...prev, [ pos ]: v } ) );
				if ( parseInt( v ) < 6 ) {
					setValue( ( prev: any ) => {
	...
	const update = ( row: string, area: string, items: any, screen = 'desktop' ) => {
	...
	const remove = ( row: string, area: string, item: any ) => {
		const newValue = cloneDeep( value );
		newValue[ device ][ row ][ area ] = ( newValue?.[ device ]?.[ row ]?.[ area ] || [] ).filter( ( v: any ) => v.id !== item.id );
		if ( ! isEqual( newValue, value ) ) {
			setValue( newValue );
			setting.set( newValue );
		}
					...
					<div className="vite-builder-portal-content-inner" data-desktop={ device }>
						{ ( 'header' === context ? [ 'desktop', 'mobile' ] : [ 'desktop' ] ).map( d => (
							<Fragment key={ d }>
								{ 'mobile' === d && (
									<div
										className="vite-builder-mobile-offset"
												...
												{ value?.offset?.map( ( item: any ) => (
													...
													<div className="vite-builder-component" key={ item.id } onClick={ choices?.mobile?.[ item.id ]?.section ? () => customizer.section( choices.mobile[ item.id ].section )?.focus() : undefined }>
														...
														<span
															...
															onClick={ () => {
																const val = cloneDeep( value );
																val.offset = val.offset.filter( ( i: any ) => i.id !== item.id );
																if ( ! isEqual( val, value ) ) {
																	setValue( val );
																	setting.set( val );
																}
								...
								<div key={ d } className="vite-builder-rows" style={ { display: d !== device ? 'none' : undefined } }>
									{
										Object.entries( areas ).map( ( [ row = '', columns = {} ] ) => (
											<div
												key={ row }
												data-row={ row }
												className="vite-builder-row"
												data-cols={ 'footer' === context ? ( col?.[ row ] ?? '' ) : undefined }
											>
												<div className="vite-builder-cols">
													{ Object.keys( columns as any ).map( ( area ) => (
														<div
															className="vite-col" data-col={ area } key={ area }
														...
														>
															<ReactSortable
																...
																animation={ 150 }
															>
																{ getAreaItems( row, area, d ).map( ( item: any ) => (
																	// @ts-ignore
																	<Item key={ item.id } { ...{
																		row,
																		area,
