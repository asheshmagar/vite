...
$options = array_reduce(
	...
	function( $acc, $curr ) {
		$acc[ "vite[footer-html-$curr]" ] = [
			...
			'type'        => 'vite-editor',
			'title'       => __( 'HTML', 'vite' ),
			'default'     => vite( 'core' )->get_mod_defaults()[ "footer-html-$curr" ],
			'partial'     => [
		...
		$acc[ "vite[footer-html-$curr-alignment]" ]  = [
			...
			'default'     => vite( 'core' )->get_mod_defaults()[ "footer-html-$curr-alignment" ],
			'partial'     => [
				'selector'        => ".vite-footer .vite-html--$curr",
				'render_callback' => function() use ( $curr ) {
					get_template_part(
		...
		$acc[ "vite[footer-html-$curr-display]" ]    = [
			...
			'default'     => vite( 'core' )->get_mod_defaults()[ "footer-html-$curr-display" ],
			'partial'     => [
				'selector'        => ".vite-footer .vite-html--$curr",
				'render_callback' => function() use ( $curr ) {
					get_template_part(
