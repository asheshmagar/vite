...
$row_options = array_reduce(
	...
	function( $acc, $curr ) use ( $core, $col_layouts ) {
		$acc[ "vite[footer-$curr-row-layout]" ] = [
			...
			'default'     => $core->get_mod_defaults()[ "footer-$curr-row-layout" ],
			'choices'     => [
		...
		$acc[ "vite[footer-$curr-row-cols]" ] = [
			...
			'default'     => $core->get_mod_defaults()[ "footer-$curr-row-cols" ],
			'partial'     => [
				'selector'            => '.vite-footer',
				'container_inclusive' => true,
				'render_callback'     => function() {
		...
		$acc[ "vite[footer-$curr-row-col-layout]" ] = [
			...
			'title'       => __( 'Layout', 'vite' ),
			'default'     => $core->get_mod_defaults()[ "footer-$curr-row-col-layout" ],
			'choices'     => array_map(
				function( $col ) {
					return array_merge(
						$col,
		...
		$acc[ "vite[footer-$curr-row-col-alignment]" ] = [
			...
			'css'         => [
				'selector' => ".vite-footer__col-$curr",
		...
		$acc[ "vite[footer-$curr-row-height]" ] = [
			...
			'default'     => $core->get_mod_defaults()[ "footer-$curr-row-height" ],
			'input_attrs' => [
				'min'        => 0,
				'max'        => 400,
				'step'       => 1,
...
$options = [
	'vite[footer]'                    => [
		...
		'default'     => $core->get_mod_default( 'footer' ),
		'priority'    => 10,
		'input_attrs' => [
			'areas'    => [
				'top'    => [
